...
Once the `conda-forge` channel has been enabled, `libopenvino, libopenvino-python, openvino` can be installed with `conda`:

```
conda install libopenvino libopenvino-python openvino
```

or with `mamba`:

```
mamba install libopenvino libopenvino-python openvino
```
